...
namespace TrainingManager.ViewModel
{
    public abstract class WorkoutManagerBaseVM : ViewModelBase
    {
        ...
        private bool IsExerciseReadyToAdd()
        {
            ...
            {
                SendPopUpMessage(MessageContent.EmptyExerciseName);
                return false;
            }

            if (NewWeightExercise.MainMuscle == Muscle.Unknown)
            {
                SendPopUpMessage(MessageContent.UnknownMuscle);
                return false;
            }

            if (NewWeightExercise.TotalExerciseWeight <= 0)
            {
                SendPopUpMessage(MessageContent.EmptyExercise);
                return false;
            }

            if (NewWeightExercise.WeightRounds.Any(x => x.WeightOfExercise <= 0))
            {
                SendPopUpMessage(MessageContent.InvalidWeight);
                return false;
            }

            if (NewWeightExercise.WeightRounds.Any(x => x.Reps <= 0))
            {
                SendPopUpMessage(MessageContent.InvalidReps);
                return false;
            }
        ...
        protected bool IsReadyReadyToSave()
        {
            ...
            {
                SendPopUpMessage(MessageContent.EmptyWorkoutName);
                return false;
            }

            if (NewWeightWorkout.TotalWeight <= 0)
            {
                SendPopUpMessage(MessageContent.EmptyWorkout);
                return false;
            }
